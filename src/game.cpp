...
void Game::genTargets(int targetAmt, int from, int to) {
    ...
    for (int i = 0; i < targetAmt; ++i) {
        ...
        targets.push_back(Target(Point(x, y), 1, 100, 10, genTargetId()));
    }
}

// Checks whether a shot has hit an existing target within its hit radius
bool Game::isTargetHit(const Point& shot, const Target& targ) {
    double distance = calcDist(shot.x, shot.y, targ.getPos().x, targ.getPos().y);
    return distance <= targ.getRadius();
}
...
void Game::recordHits(const Point& shot) {
    for (unsigned int i = 0u; i < targets.size(); ++i) {
        Target t = targets[i];
        // TODO: Make a class responsible for hit detection
        if (t.getHP() > 0 && isTargetHit(shot, t)) {
            points += t.getPoints();
            t.changeHP(-100);
        }
    }
